  ...
  return (
    <>
      <header className={`fixed top-0 z-50 w-full transition-all duration-300 ${
        isTransparent 
          ? "dark bg-gradient-to-b from-black/80 via-black/30 to-transparent border-transparent" 
        ...
        <div className="mx-auto flex max-w-[1400px] h-20 items-center justify-between px-4 md:px-8">
          ...
          <Link href="/" className="flex items-center group">
            <img src="/logo.png" alt="ComiHarth" className={`h-16 w-auto object-contain transition-transform duration-300 group-hover:scale-105 ${
              isTransparent ? "invert brightness-0" : "dark:invert"
            }`} />
          </Link>
          ...
          <form onSubmit={handleSearchSubmit} className="hidden md:flex relative w-80">
            <input
              ...
              className={`w-full h-10 rounded-full pl-10 pr-4 text-sm focus:outline-none focus:border-accent-green/70 focus:ring-1 focus:ring-accent-green/70 transition-all ${
                isTransparent 
                  ? "bg-white/10 border-white/20 text-white placeholder-white/60 focus:bg-black/40 backdrop-blur-md" 
                  : "bg-surface border-border-dark/70 text-foreground placeholder-muted-text/60"
              }`}
            />
            <Search className={`absolute left-3.5 top-3 h-4 w-4 ${isTransparent ? 'text-white/60' : 'text-muted-text/60'}`} />
          </form>
