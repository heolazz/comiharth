        ...
        <div className="flex flex-col gap-3">
          ...
          >
            <h1 className="text-4xl md:text-6xl font-display font-black text-foreground tracking-tight flex items-center gap-3">
              ...
              >
                <Sparkles className="h-8 w-8 md:h-12 md:w-12 text-yellow-400 drop-shadow-lg" />
              </motion.div>
            </h1>
          </motion.div>
          <motion.p 
          ...
          <AnimatePresence>
            {historyList.map((item, index) => (
              <motion.div
                ...
                className="group flex flex-col rounded-[32px] border-2 border-border-dark/40 bg-surface hover:bg-surface-hover hover:border-accent-green/50 transition-all duration-300 shadow-sm hover:shadow-2xl hover:shadow-accent-green/10 overflow-hidden relative cursor-pointer"
              >
                {/* Image Section - Bouncy */}
                <div className="relative w-full h-48 sm:h-56 overflow-hidden bg-background">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-surface via-surface/40 to-transparent opacity-80 group-hover:opacity-60 transition-opacity" />
                  
                  {/* Floating Action Button */}
                  <Link
                    href={`/read/${item.provider}/${item.chapterId}`}
                    className="absolute bottom-4 right-4 flex items-center justify-center h-14 w-14 rounded-full bg-accent-green hover:bg-green-600 text-white shadow-xl shadow-black/50 transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 hover:scale-110 z-20"
                  ...
                  {/* Chapter Badge */}
                  <div className="absolute top-4 left-4 bg-background/90 backdrop-blur-md border border-border-dark/50 px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-lg z-10 transform -rotate-2 group-hover:rotate-0 transition-transform">
                ...
                <Link 
                  ...
                  className="flex flex-col flex-grow p-5 md:p-6 justify-between min-w-0 z-10 relative bg-surface group-hover:bg-surface-hover transition-colors"
                >
                  <div className="flex flex-col gap-2">
                    <h3 className="text-lg font-black text-foreground group-hover:text-accent-green transition-colors line-clamp-2 leading-tight">
                      {item.comicTitle}
                    </h3>
                  </div>

                  <div className="flex items-center justify-between mt-6 pt-4 border-t border-border-dark/40">
                    <span className="text-xs text-muted-text/80 font-bold flex items-center gap-1.5 bg-background/50 px-2 py-1 rounded-lg">
